...
| Coordinator model bakeoff | `docs/planning/coordinator-model-bakeoff.md`, future local eval fixtures | Local model evaluation prompts only; no tools by design | Uses sanitized ChatBoks snippets and synthetic fixtures; live results should stay under `.chatboks/evals/` | Loopback/local runtime only unless separately reviewed | Must not include secrets, provider dashboards, or cross-project memory without explicit consent. Does not authorize cloud fallback. | ChatBoks planning only | 2026-06-15 | Medium: local prompt privacy, model-selection influence |
