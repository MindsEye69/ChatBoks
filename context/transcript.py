...
TRANSCRIPT_AGENT_TAGS = r"YOU|CLAUDE|CODEX|CODEX_SPARK|COORDINATOR|ANTIGRAVITY|ANTIGRAV|SYSTEM"
TRANSCRIPT_TURN_RE = re.compile(
    rf"^\[({TRANSCRIPT_AGENT_TAGS})\]",
    re.I,
)
SUMMARY_CHECKPOINT_START_RE = re.compile(
    rf"^(?:\[({TRANSCRIPT_AGENT_TAGS})\]\s+)?>>> SUMMARY_CHECKPOINT\b",
    re.I,
)
