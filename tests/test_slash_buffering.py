...
def test_handle_user_input_routes_slash_commands_without_buffer():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # /usage should stay local and not trigger a round
        ...
        app.handle_graph_command = MagicMock()
        app.handle_user_input("/graph")
        app.handle_graph_command.assert_called_once()
        app.run_agent_round.assert_not_called()

        # /resume should stay local and not trigger a round
        app.handle_resume_command = MagicMock()
        app.handle_user_input("/resume")
        app.handle_resume_command.assert_called_once()
        app.run_agent_round.assert_not_called()

        # /health should stay local and not trigger a round
        app.handle_trajectory_health_command = MagicMock()
        app.handle_user_input("/health")
        app.handle_trajectory_health_command.assert_called_once_with("/health")
        app.run_agent_round.assert_not_called()
        ...
        app.handle_sleep_command = MagicMock()
        app.handle_user_input("/sleep")
        app.handle_sleep_command.assert_called_once_with("/sleep")
        app.run_agent_round.assert_not_called()

        # /model-commands should stay local and not trigger a round
        app.handle_model_commands_command = MagicMock()
        app.handle_user_input("/model-commands")
        app.handle_model_commands_command.assert_called_once()
        app.run_agent_round.assert_not_called()
