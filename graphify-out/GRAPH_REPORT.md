# Graph Report - chatboks  (2026-07-04)

## Corpus Check
- 124 files · ~436,480 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2689 nodes · 5295 edges · 179 communities (163 shown, 16 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 292 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0a0045c9`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_Community 0|Community 0]]
- [[_COMMUNITY_Community 1|Community 1]]
- [[_COMMUNITY_Community 2|Community 2]]
- [[_COMMUNITY_Community 3|Community 3]]
- [[_COMMUNITY_Community 4|Community 4]]
- [[_COMMUNITY_Community 5|Community 5]]
- [[_COMMUNITY_Community 6|Community 6]]
- [[_COMMUNITY_Community 7|Community 7]]
- [[_COMMUNITY_Community 8|Community 8]]
- [[_COMMUNITY_Community 9|Community 9]]
- [[_COMMUNITY_Community 10|Community 10]]
- [[_COMMUNITY_Community 11|Community 11]]
- [[_COMMUNITY_Community 12|Community 12]]
- [[_COMMUNITY_Community 13|Community 13]]
- [[_COMMUNITY_Community 14|Community 14]]
- [[_COMMUNITY_Community 15|Community 15]]
- [[_COMMUNITY_Community 16|Community 16]]
- [[_COMMUNITY_Community 17|Community 17]]
- [[_COMMUNITY_Community 18|Community 18]]
- [[_COMMUNITY_Community 19|Community 19]]
- [[_COMMUNITY_Community 20|Community 20]]
- [[_COMMUNITY_Community 21|Community 21]]
- [[_COMMUNITY_Community 22|Community 22]]
- [[_COMMUNITY_Community 23|Community 23]]
- [[_COMMUNITY_Community 24|Community 24]]
- [[_COMMUNITY_Community 25|Community 25]]
- [[_COMMUNITY_Community 26|Community 26]]
- [[_COMMUNITY_Community 27|Community 27]]
- [[_COMMUNITY_Community 42|Community 42]]
- [[_COMMUNITY_Community 43|Community 43]]
- [[_COMMUNITY_Community 44|Community 44]]
- [[_COMMUNITY_Community 46|Community 46]]
- [[_COMMUNITY_Community 47|Community 47]]
- [[_COMMUNITY_Community 48|Community 48]]
- [[_COMMUNITY_Community 49|Community 49]]
- [[_COMMUNITY_Community 50|Community 50]]
- [[_COMMUNITY_Community 51|Community 51]]
- [[_COMMUNITY_Community 52|Community 52]]
- [[_COMMUNITY_Community 56|Community 56]]
- [[_COMMUNITY_Community 57|Community 57]]
- [[_COMMUNITY_Community 58|Community 58]]
- [[_COMMUNITY_Community 59|Community 59]]
- [[_COMMUNITY_Community 60|Community 60]]
- [[_COMMUNITY_Community 61|Community 61]]
- [[_COMMUNITY_Community 62|Community 62]]
- [[_COMMUNITY_Community 63|Community 63]]
- [[_COMMUNITY_Community 64|Community 64]]
- [[_COMMUNITY_Community 65|Community 65]]
- [[_COMMUNITY_Community 66|Community 66]]
- [[_COMMUNITY_Community 67|Community 67]]
- [[_COMMUNITY_Community 68|Community 68]]
- [[_COMMUNITY_Community 69|Community 69]]
- [[_COMMUNITY_Community 70|Community 70]]
- [[_COMMUNITY_Community 71|Community 71]]
- [[_COMMUNITY_Community 72|Community 72]]
- [[_COMMUNITY_Community 73|Community 73]]
- [[_COMMUNITY_Community 74|Community 74]]
- [[_COMMUNITY_Community 75|Community 75]]
- [[_COMMUNITY_Community 76|Community 76]]
- [[_COMMUNITY_Community 77|Community 77]]
- [[_COMMUNITY_Community 78|Community 78]]
- [[_COMMUNITY_Community 79|Community 79]]
- [[_COMMUNITY_Community 80|Community 80]]
- [[_COMMUNITY_Community 81|Community 81]]
- [[_COMMUNITY_Community 82|Community 82]]
- [[_COMMUNITY_Community 83|Community 83]]
- [[_COMMUNITY_Community 84|Community 84]]
- [[_COMMUNITY_Community 85|Community 85]]
- [[_COMMUNITY_Community 86|Community 86]]
- [[_COMMUNITY_Community 87|Community 87]]
- [[_COMMUNITY_Community 88|Community 88]]
- [[_COMMUNITY_Community 89|Community 89]]
- [[_COMMUNITY_Community 90|Community 90]]
- [[_COMMUNITY_Community 91|Community 91]]
- [[_COMMUNITY_Community 92|Community 92]]
- [[_COMMUNITY_Community 93|Community 93]]
- [[_COMMUNITY_Community 94|Community 94]]
- [[_COMMUNITY_Community 95|Community 95]]
- [[_COMMUNITY_Community 96|Community 96]]
- [[_COMMUNITY_Community 97|Community 97]]
- [[_COMMUNITY_Community 98|Community 98]]
- [[_COMMUNITY_Community 99|Community 99]]
- [[_COMMUNITY_Community 100|Community 100]]
- [[_COMMUNITY_Community 101|Community 101]]
- [[_COMMUNITY_Community 102|Community 102]]
- [[_COMMUNITY_Community 103|Community 103]]
- [[_COMMUNITY_Community 104|Community 104]]
- [[_COMMUNITY_Community 105|Community 105]]
- [[_COMMUNITY_Community 106|Community 106]]
- [[_COMMUNITY_Community 107|Community 107]]
- [[_COMMUNITY_Community 108|Community 108]]
- [[_COMMUNITY_Community 109|Community 109]]
- [[_COMMUNITY_Community 110|Community 110]]
- [[_COMMUNITY_Community 111|Community 111]]
- [[_COMMUNITY_Community 112|Community 112]]
- [[_COMMUNITY_Community 113|Community 113]]
- [[_COMMUNITY_Community 114|Community 114]]
- [[_COMMUNITY_Community 115|Community 115]]
- [[_COMMUNITY_Community 116|Community 116]]
- [[_COMMUNITY_Community 117|Community 117]]
- [[_COMMUNITY_Community 118|Community 118]]
- [[_COMMUNITY_Community 119|Community 119]]
- [[_COMMUNITY_Community 120|Community 120]]
- [[_COMMUNITY_Community 121|Community 121]]
- [[_COMMUNITY_Community 122|Community 122]]
- [[_COMMUNITY_Community 123|Community 123]]
- [[_COMMUNITY_Community 124|Community 124]]
- [[_COMMUNITY_Community 125|Community 125]]
- [[_COMMUNITY_Community 126|Community 126]]
- [[_COMMUNITY_Community 127|Community 127]]
- [[_COMMUNITY_Community 128|Community 128]]
- [[_COMMUNITY_Community 129|Community 129]]
- [[_COMMUNITY_Community 130|Community 130]]
- [[_COMMUNITY_Community 131|Community 131]]
- [[_COMMUNITY_Community 132|Community 132]]
- [[_COMMUNITY_Community 133|Community 133]]
- [[_COMMUNITY_Community 134|Community 134]]
- [[_COMMUNITY_Community 135|Community 135]]
- [[_COMMUNITY_Community 136|Community 136]]
- [[_COMMUNITY_Community 137|Community 137]]
- [[_COMMUNITY_Community 138|Community 138]]
- [[_COMMUNITY_Community 140|Community 140]]
- [[_COMMUNITY_Community 141|Community 141]]
- [[_COMMUNITY_Community 142|Community 142]]
- [[_COMMUNITY_Community 143|Community 143]]
- [[_COMMUNITY_Community 144|Community 144]]
- [[_COMMUNITY_Community 145|Community 145]]
- [[_COMMUNITY_Community 146|Community 146]]
- [[_COMMUNITY_Community 147|Community 147]]
- [[_COMMUNITY_Community 148|Community 148]]
- [[_COMMUNITY_Community 149|Community 149]]
- [[_COMMUNITY_Community 150|Community 150]]
- [[_COMMUNITY_Community 151|Community 151]]
- [[_COMMUNITY_Community 152|Community 152]]
- [[_COMMUNITY_Community 153|Community 153]]
- [[_COMMUNITY_Community 154|Community 154]]
- [[_COMMUNITY_Community 155|Community 155]]
- [[_COMMUNITY_Community 156|Community 156]]
- [[_COMMUNITY_Community 157|Community 157]]
- [[_COMMUNITY_Community 158|Community 158]]
- [[_COMMUNITY_Community 159|Community 159]]
- [[_COMMUNITY_Community 160|Community 160]]
- [[_COMMUNITY_Community 161|Community 161]]
- [[_COMMUNITY_Community 162|Community 162]]
- [[_COMMUNITY_Community 163|Community 163]]
- [[_COMMUNITY_Community 164|Community 164]]
- [[_COMMUNITY_Community 165|Community 165]]
- [[_COMMUNITY_Community 166|Community 166]]
- [[_COMMUNITY_Community 167|Community 167]]
- [[_COMMUNITY_Community 168|Community 168]]
- [[_COMMUNITY_Community 169|Community 169]]
- [[_COMMUNITY_Community 170|Community 170]]
- [[_COMMUNITY_Community 171|Community 171]]
- [[_COMMUNITY_Community 172|Community 172]]
- [[_COMMUNITY_Community 173|Community 173]]
- [[_COMMUNITY_Community 174|Community 174]]
- [[_COMMUNITY_Community 175|Community 175]]
- [[_COMMUNITY_Community 176|Community 176]]
- [[_COMMUNITY_Community 177|Community 177]]
- [[_COMMUNITY_Community 178|Community 178]]

## God Nodes (most connected - your core abstractions)
1. `Chatboks` - 266 edges
2. `Stream` - 99 edges
3. `CoordinatorAgent` - 85 edges
4. `Router` - 70 edges
5. `ChatboksTextualApp` - 66 edges
6. `ContextBuilder` - 62 edges
7. `_router()` - 59 edges
8. `Any` - 54 edges
9. `Path` - 46 edges
10. `BaseAgent` - 39 edges

## Surprising Connections (you probably didn't know these)
- `Namespace` --uses--> `CoordinatorAgent`  [INFERRED]
  coordinator_bakeoff.py → agents/coordinator.py
- `Chatboks` --uses--> `Chatboks`  [INFERRED]
  tests/test_latency.py → orchestrator.py
- `Chatboks` --uses--> `Chatboks`  [INFERRED]
  tests/test_outcomes.py → orchestrator.py
- `Chatboks` --uses--> `Chatboks`  [INFERRED]
  tests/test_skills.py → orchestrator.py
- `Chatboks` --uses--> `Chatboks`  [INFERRED]
  tests/test_slash_buffering.py → orchestrator.py

## Import Cycles
- 1-file cycle: `orchestrator.py -> orchestrator.py`
- 1-file cycle: `tests/test_direct_agents.py -> tests/test_direct_agents.py`

## Communities (179 total, 16 thin omitted)

### Community 1 - "Community 1"
Cohesion: 0.06
Nodes (67): AntigravityAgent, BaseAgent, Any, Raised when an agent CLI rejects a prompt because the context is too large., TokenExhaustionError, ClaudeAgent, CodexAgent, CodexSparkAgent (+59 more)

### Community 2 - "Community 2"
Cohesion: 0.07
Nodes (34): Connection, ContextBuilder, Any, Path, Any, Path, Small deterministic fallback summarizer.      The design allows this to become a, Summarizer (+26 more)

### Community 3 - "Community 3"
Cohesion: 0.06
Nodes (8): test_stream_help_box_contains_bbs_frame_and_commands(), test_stream_help_pin_contains_compact_commands(), test_stream_standby_uses_agent_label(), test_stream_token_usage_renders_session_bars(), Tests for terminal intro rendering., test_agent_output_uses_visible_answer_rules(), Text, Stream

### Community 4 - "Community 4"
Cohesion: 0.05
Nodes (53): CoordinatorAgent, Any, Path, Return the agents that should handle a user prompt.          A leading @agent pr, Router, FakeJsonResponse, _make_router(), Path (+45 more)

### Community 5 - "Community 5"
Cohesion: 0.04
Nodes (41): Credits, Workflow Inspirations, Agent Availability, ChatBoks, CodeGraph (third-party integration), Collaboration Modes, Cost Estimates, Early Development Notice (+33 more)

### Community 6 - "Community 6"
Cohesion: 0.06
Nodes (60): _config(), Dedicated unit tests for Router and RoutingDecision., _router(), test_after_returns_next_agent(), test_after_returns_you_at_end_of_list(), test_after_unknown_agent_returns_primary(), test_auto_route_claude_for_analysis_prompt(), test_auto_route_claude_suppressed_when_prompt_contains_implement() (+52 more)

### Community 7 - "Community 7"
Cohesion: 0.16
Nodes (38): _approval_dir(), Path, Tests for F3: trusted role-file approval (trust.py).  Covers: - First approval:, test_approval_pin_is_stored_outside_project(), test_first_approval_interactive_hash_stored_after_approval(), test_first_approval_interactive_user_approves_returns_content(), test_hash_match_non_interactive_returns_content(), test_hash_match_returns_content_without_prompting() (+30 more)

### Community 8 - "Community 8"
Cohesion: 0.12
Nodes (24): ContextBuilder, CoordinatorAgent, _make_app(), _make_builder(), _make_coordinator(), Chatboks, Path, Smoke tests for security hardening pass 1.  Covers: - chatboks.md history marked (+16 more)

### Community 9 - "Community 9"
Cohesion: 0.18
Nodes (10): context, expected, must_avoid, must_include, preferred_response_shape, scoring_focus, id, schema_version (+2 more)

### Community 10 - "Community 10"
Cohesion: 0.12
Nodes (40): RoutingDecision, _make_app(), Chatboks, Path, Smoke tests for agent availability and exhausted-agent routing., test_agent_command_marks_exhausted_without_agent_round(), test_coordinator_can_substitute_after_main_agents_exhausted_when_allowed(), test_direct_fallback_must_be_allowed_to_fill_main_seat() (+32 more)

### Community 12 - "Community 12"
Cohesion: 0.23
Nodes (11): _make_app(), Smoke test: PROPOSAL from non-last agent is buffered; all agents run before user, Return a Chatboks instance with all I/O mocked out., QUESTION from second agent returns immediately, abandoning buffered PROPOSAL., A weak trailing agent must not override a prior completed result., First agent PROPOSAL must not short-circuit; second agent must still run., Single-agent project: PROPOSAL fires in post-loop handler (unchanged behavior)., test_blocked_after_prior_completion_is_warning_not_terminal_block() (+3 more)

### Community 13 - "Community 13"
Cohesion: 0.18
Nodes (9): Event, RemoteEventBuffer, RemoteBridgeServer, BlockingFakeApp, FakeStream, test_remote_event_buffer_preserves_stream_delta_whitespace(), test_remote_session_snapshot_includes_compact_proposal(), test_remote_session_snapshot_includes_trace_payload() (+1 more)

### Community 14 - "Community 14"
Cohesion: 0.33
Nodes (5): Context Priming, Escalation Triggers, Implement Mode, Quality Gate, Workflow

### Community 15 - "Community 15"
Cohesion: 0.60
Nodes (5): Bugsearch Mode, Context Priming, Escalation Triggers, Quality Gate, Workflow

### Community 16 - "Community 16"
Cohesion: 0.15
Nodes (33): FakeSession, Path, run_server(), test_codegraph_stats_reads_counts_from_database(), test_git_environment_returns_none_for_non_repo(), test_operator_file_guard_ignores_stale_operator_file(), test_operator_file_guard_rejects_active_bridge(), test_packet_trace_reads_compact_packet_records() (+25 more)

### Community 17 - "Community 17"
Cohesion: 0.14
Nodes (13): dependencies, @capacitor/android, @capacitor/core, description, devDependencies, @capacitor/cli, name, private (+5 more)

### Community 18 - "Community 18"
Cohesion: 0.09
Nodes (42): apiFetch(), appendStreamText(), applyEventToList(), applySession(), clearSendStatusSoon(), copyLatestResponse(), currentBaseUrl(), currentPairCode() (+34 more)

### Community 19 - "Community 19"
Cohesion: 0.07
Nodes (65): agentDisplayName(), apiFetch(), appendStreamText(), applyEventToList(), applySession(), canonicalAgent(), clearSendStatusSoon(), copyLatestResponse() (+57 more)

### Community 20 - "Community 20"
Cohesion: 0.25
Nodes (7): appId, appName, server, allowNavigation, androidScheme, cleartext, webDir

### Community 21 - "Community 21"
Cohesion: 0.25
Nodes (7): Adversarial Pressure, ChatBoks Collaboration Protocol, Contribution Stance, Coordination Rules, Evidence Standard, Signals, Thought Packets

### Community 22 - "Community 22"
Cohesion: 0.25
Nodes (7): appId, appName, server, allowNavigation, androidScheme, cleartext, webDir

### Community 23 - "Community 23"
Cohesion: 0.33
Nodes (5): Android build, ChatBoks Remote Android Shell, Desktop side, Early Development Notice, Notes

### Community 24 - "Community 24"
Cohesion: 0.33
Nodes (5): Boundaries, Codex's Role - ChatBoks, Collaboration Duties, Primary Lane, Tooling Safety

### Community 42 - "Community 42"
Cohesion: 0.18
Nodes (10): context, expected, must_avoid, must_include, preferred_response_shape, scoring_focus, id, schema_version (+2 more)

### Community 43 - "Community 43"
Cohesion: 0.40
Nodes (4): CodeGraph, Freshness Checks, Graph Workflow, Graphify

### Community 46 - "Community 46"
Cohesion: 0.08
Nodes (7): Changed, ComposeResult, Exception, test_textual_app_agent_summary_uses_project_state(), test_textual_app_completion_palette_covers_fixed_choice_commands(), ChatboksTextualApp, Optional Textual shell for interactive ChatBoks sessions.

### Community 47 - "Community 47"
Cohesion: 0.10
Nodes (41): AgentTimeoutError, Path, Raised when an agent CLI exceeds its idle or wall-clock timeout., AgentTimeoutError, FakeAgent, _make_app(), Chatboks, Path (+33 more)

### Community 48 - "Community 48"
Cohesion: 0.14
Nodes (21): append_result(), build_prompt(), chat_completions_url(), extract_chat_output(), load_fixtures(), main(), model_slug(), parse_args() (+13 more)

### Community 49 - "Community 49"
Cohesion: 0.14
Nodes (3): RemoteSession, RemoteStream, Stream

### Community 50 - "Community 50"
Cohesion: 0.33
Nodes (5): Boundaries, Codex Spark's Role - ChatBoks, Collaboration Duties, Primary Lane, Tooling Safety

### Community 51 - "Community 51"
Cohesion: 0.29
Nodes (5): HTTPStatus, is_allowed_app_origin(), parse_query_int(), RemoteHandler, test_allowed_app_origin_accepts_localhost_with_optional_ports()

### Community 56 - "Community 56"
Cohesion: 0.13
Nodes (15): Offset, FakeTextualApp, Path, test_textual_app_completion_palette_keyboard_navigation(), test_textual_app_copies_local_transcript_selection(), test_textual_app_copies_transcript_without_selection(), test_textual_app_does_not_copy_without_selection(), test_textual_app_mounts_with_fake_chatboks() (+7 more)

### Community 57 - "Community 57"
Cohesion: 0.12
Nodes (16): 2026-06-15 Post-Normalization Run, Candidate Models, Coordinator Model Bakeoff, Current Baseline, Evaluation Tasks, Goal, Implementation Plan, Manual Runner (+8 more)

### Community 58 - "Community 58"
Cohesion: 0.12
Nodes (33): configure_utf8_stdio(), Return an environment that makes Python children speak UTF-8., Prefer UTF-8 for ChatBoks console/process I/O on every platform., utf8_env(), append_commit_message(), git_output(), load_state(), main() (+25 more)

### Community 59 - "Community 59"
Cohesion: 0.18
Nodes (10): ChatBoks Interface Handoff, Current Mobile Remote UI Notes, Current Repo State, Fresh Chat Prompt, Interface Direction, Security Scan Findings To Keep In Mind, Suggested Next Work, Terminal / Mini-Terminal Idea (+2 more)

### Community 61 - "Community 61"
Cohesion: 0.18
Nodes (10): context, expected, must_avoid, must_include, preferred_response_shape, scoring_focus, id, schema_version (+2 more)

### Community 62 - "Community 62"
Cohesion: 0.20
Nodes (9): ChatBoks Mobile Remote Pairing Runbook, Generate A New Pairing Code, How Pairing Works, If The Command Cannot Find The Operator File, Public Safety, Relevant Tests, Start The Bridge, Troubleshooting (+1 more)

### Community 63 - "Community 63"
Cohesion: 0.06
Nodes (78): AGENT_GLYPHS, AGENT_LABELS, agentDisplayName(), agentGlyph(), agentIsLive(), agentStatusValue(), apiFetch(), apiUrl() (+70 more)

### Community 64 - "Community 64"
Cohesion: 0.09
Nodes (22): Allowed Projects, ChatBoks Tool Trust Ledger and Privacy Contract, Cloud Fallback, Coordinator, Coordinator Summaries and Transcript Compaction, Description-Code Consistency Review, Graphify, Local Models (+14 more)

### Community 65 - "Community 65"
Cohesion: 0.14
Nodes (36): agent_can_fill_lane(), agent_is_live(), agent_status_value(), agent_trace_from_transcript(), append_lane_agent(), build_lane_agents(), build_token_usage(), canonical_agent_config() (+28 more)

### Community 66 - "Community 66"
Cohesion: 0.18
Nodes (10): context, expected, must_avoid, must_include, preferred_response_shape, scoring_focus, id, schema_version (+2 more)

### Community 68 - "Community 68"
Cohesion: 0.33
Nodes (5): Boundaries, Claude's Role - ChatBoks, Collaboration Duties, Primary Lane, Tooling Safety

### Community 69 - "Community 69"
Cohesion: 0.40
Nodes (3): promptInput, terminalFocus, themeToggle

### Community 71 - "Community 71"
Cohesion: 0.18
Nodes (10): context, expected, must_avoid, must_include, preferred_response_shape, scoring_focus, id, schema_version (+2 more)

### Community 72 - "Community 72"
Cohesion: 0.50
Nodes (3): Coordinator's Role - ChatBoks, Output Rules, Scope

### Community 73 - "Community 73"
Cohesion: 0.12
Nodes (12): BaseHTTPRequestHandler, default_operator_status_path(), is_allowed_bind_host(), is_loopback_host(), is_tailnet_ipv4_host(), main(), RemoteAuth, RemoteBridgeServer (+4 more)

### Community 75 - "Community 75"
Cohesion: 0.18
Nodes (10): context, expected, must_avoid, must_include, preferred_response_shape, scoring_focus, id, schema_version (+2 more)

### Community 76 - "Community 76"
Cohesion: 0.18
Nodes (10): context, expected, must_avoid, must_include, preferred_response_shape, scoring_focus, id, schema_version (+2 more)

### Community 77 - "Community 77"
Cohesion: 0.16
Nodes (3): Stream, Stream adapter that writes ChatBoks events into a Textual RichLog., TextualStream

### Community 78 - "Community 78"
Cohesion: 0.15
Nodes (22): append_result(), build_prompt(), coordinator_config(), load_config(), load_fixtures(), main(), model_slug(), ollama_payload() (+14 more)

### Community 79 - "Community 79"
Cohesion: 0.09
Nodes (22): attention, codeGraph, configured, issue, status, customModules, dashboardMarkdownExists, files (+14 more)

### Community 80 - "Community 80"
Cohesion: 0.24
Nodes (12): clean_list(), has_source_anchor(), normalize_value(), packet_records_from_jsonl(), parse_packet_body(), Any, split_observed_by_anchor(), ThoughtPacket (+4 more)

### Community 81 - "Community 81"
Cohesion: 0.25
Nodes (20): _make_app(), Chatboks, Path, Smoke tests for the local /help command., test_codegraph_status_lines_reads_sqlite_counts(), test_graph_command_renders_without_agent_round(), test_graphify_status_lines_reports_fresh_graph(), test_graphify_status_lines_reports_stale_graph() (+12 more)

### Community 83 - "Community 83"
Cohesion: 0.09
Nodes (22): attention, customModules, dashboardMarkdownExists, files, generatedAt, graphify, configured, issue (+14 more)

### Community 84 - "Community 84"
Cohesion: 0.11
Nodes (17): Browser / Playwright Automation, CodeGraph MCP, Description-Code Consistency Gate, Entry Template, Foundry Local Candidate Runtime, Goal, Hugging Face Connector, Initial Entries (+9 more)

### Community 86 - "Community 86"
Cohesion: 0.12
Nodes (16): flows, program, sessionClose, sessionStart, edges, nodes, positions, title (+8 more)

### Community 87 - "Community 87"
Cohesion: 0.19
Nodes (8): MouseDown, MouseMove, MouseUp, RichLog, Selection, Strip, RichLog variant that paints and extracts Textual text selections., SelectableRichLog

### Community 88 - "Community 88"
Cohesion: 0.12
Nodes (16): flows, program, sessionClose, sessionStart, edges, nodes, positions, title (+8 more)

### Community 89 - "Community 89"
Cohesion: 0.23
Nodes (5): Key, OptionSelected, TextArea, PromptTextArea, Four-line prompt that submits on Enter.

### Community 90 - "Community 90"
Cohesion: 0.14
Nodes (14): modulesDir, projects, config, checks, data, home, modulesDir, path (+6 more)

### Community 91 - "Community 91"
Cohesion: 0.14
Nodes (14): assignmentStore, localUser, participants, updatedAt, id, name, ticketStore, collaboration (+6 more)

### Community 92 - "Community 92"
Cohesion: 0.14
Nodes (13): Approach Comparison, Baseline Prompts, Candidate Workflows, Decision, Evaluation Plan, Held-Out Scenarios, Judge Rubric, Open Questions (+5 more)

### Community 93 - "Community 93"
Cohesion: 0.14
Nodes (14): modulesDir, projects, config, checks, data, home, modulesDir, path (+6 more)

### Community 94 - "Community 94"
Cohesion: 0.14
Nodes (14): assignmentStore, localUser, participants, updatedAt, id, name, ticketStore, collaboration (+6 more)

### Community 95 - "Community 95"
Cohesion: 0.17
Nodes (13): paperSleuth, paperSleuth, exists, openTicketCount, projectFolder, projectRecentlyCompleted, projectTicketsPath, recentlyCompleted (+5 more)

### Community 96 - "Community 96"
Cohesion: 0.18
Nodes (13): collaboration, dropToken, dropTokenConfigured, enabled, endpoint, peerName, reservations, reservationStore (+5 more)

### Community 97 - "Community 97"
Cohesion: 0.17
Nodes (13): paperSleuth, paperSleuth, exists, openTicketCount, projectFolder, projectRecentlyCompleted, projectTicketsPath, recentlyCompleted (+5 more)

### Community 98 - "Community 98"
Cohesion: 0.11
Nodes (20): collaboration, dropToken, dropTokenConfigured, dropZone, enabled, endpoint, peerName, reservations (+12 more)

### Community 99 - "Community 99"
Cohesion: 0.18
Nodes (10): Attention Queue, Close Readiness, Custom Modules, Dashboard Data, Git, Next Session Brief, Paper Sleuth Tickets, Project Dashboard (+2 more)

### Community 100 - "Community 100"
Cohesion: 0.18
Nodes (10): context, expected, must_avoid, must_include, preferred_response_shape, scoring_focus, id, schema_version (+2 more)

### Community 101 - "Community 101"
Cohesion: 0.18
Nodes (10): context, expected, must_avoid, must_include, preferred_response_shape, scoring_focus, id, schema_version (+2 more)

### Community 102 - "Community 102"
Cohesion: 0.18
Nodes (10): context, expected, must_avoid, must_include, preferred_response_shape, scoring_focus, id, schema_version (+2 more)

### Community 103 - "Community 103"
Cohesion: 0.18
Nodes (10): context, expected, must_avoid, must_include, preferred_response_shape, scoring_focus, id, schema_version (+2 more)

### Community 104 - "Community 104"
Cohesion: 0.18
Nodes (10): context, expected, must_avoid, must_include, preferred_response_shape, scoring_focus, id, schema_version (+2 more)

### Community 105 - "Community 105"
Cohesion: 0.20
Nodes (10): brief, generatedAt, headline, items, metrics, collaboratorsOnline, dirtyWorktrees, handoffs (+2 more)

### Community 106 - "Community 106"
Cohesion: 0.20
Nodes (9): Evaluation Tasks, First Target, Goal, Implementation Notes, Lane Model Bakeoff, Manual Runner, Privacy Rules, Scoring Rubric (+1 more)

### Community 107 - "Community 107"
Cohesion: 0.20
Nodes (10): brief, generatedAt, headline, items, metrics, collaboratorsOnline, dirtyWorktrees, handoffs (+2 more)

### Community 108 - "Community 108"
Cohesion: 0.33
Nodes (7): _make_app(), Chatboks, Path, Smoke tests for usage baseline syncing and summaries., test_usage_show_reports_no_records_yet(), test_usage_summary_reads_saved_jsonl(), test_usage_sync_records_playwright_capture_metadata()

### Community 109 - "Community 109"
Cohesion: 0.22
Nodes (9): appControl, available, command, label, mode, pid, processName, running (+1 more)

### Community 110 - "Community 110"
Cohesion: 0.22
Nodes (9): appControl, available, command, label, mode, pid, processName, running (+1 more)

### Community 111 - "Community 111"
Cohesion: 0.25
Nodes (8): git, aheadBehind, branch, clean, commits, remoteUrl, status, uncommitted

### Community 112 - "Community 112"
Cohesion: 0.25
Nodes (8): git, aheadBehind, branch, clean, commits, remoteUrl, status, uncommitted

### Community 113 - "Community 113"
Cohesion: 0.11
Nodes (18): attention, customModules, dashboardMarkdownExists, files, generatedAt, handoffs, mode, notes (+10 more)

### Community 114 - "Community 114"
Cohesion: 0.18
Nodes (13): collaboration, dropToken, dropTokenConfigured, enabled, endpoint, peerName, reservations, reservationStore (+5 more)

### Community 115 - "Community 115"
Cohesion: 0.48
Nodes (6): Path, _read(), test_tool_ledger_documents_attested_mcp_admission_controls(), test_tool_ledger_documents_group_and_mutation_controls(), test_trust_contract_denies_unadmitted_mcp_servers_by_default(), test_trust_contract_documents_protocol_readiness_controls()

### Community 116 - "Community 116"
Cohesion: 0.33
Nodes (6): fileTree, children, name, path, type, uncommitted

### Community 117 - "Community 117"
Cohesion: 0.33
Nodes (6): fileTree, children, name, path, type, uncommitted

### Community 118 - "Community 118"
Cohesion: 0.40
Nodes (5): procedures, close, dir, json, startup

### Community 119 - "Community 119"
Cohesion: 0.40
Nodes (5): project, id, name, path, ticketFolder

### Community 120 - "Community 120"
Cohesion: 0.40
Nodes (5): stats, files, languages, linesOfCode, sessions

### Community 121 - "Community 121"
Cohesion: 0.40
Nodes (5): procedures, close, dir, json, startup

### Community 122 - "Community 122"
Cohesion: 0.40
Nodes (5): project, id, name, path, ticketFolder

### Community 123 - "Community 123"
Cohesion: 0.40
Nodes (5): stats, files, languages, linesOfCode, sessions

### Community 125 - "Community 125"
Cohesion: 0.12
Nodes (16): flows, program, sessionClose, sessionStart, edges, nodes, positions, title (+8 more)

### Community 126 - "Community 126"
Cohesion: 0.14
Nodes (14): modulesDir, projects, config, checks, data, home, modulesDir, path (+6 more)

### Community 128 - "Community 128"
Cohesion: 0.14
Nodes (14): assignmentStore, localUser, participants, updatedAt, id, name, ticketStore, collaboration (+6 more)

### Community 130 - "Community 130"
Cohesion: 0.09
Nodes (22): attention, codeGraph, configured, issue, status, customModules, dashboardMarkdownExists, files (+14 more)

### Community 131 - "Community 131"
Cohesion: 0.17
Nodes (13): paperSleuth, paperSleuth, exists, openTicketCount, projectFolder, projectRecentlyCompleted, projectTicketsPath, recentlyCompleted (+5 more)

### Community 132 - "Community 132"
Cohesion: 0.12
Nodes (16): flows, program, sessionClose, sessionStart, edges, nodes, positions, title (+8 more)

### Community 133 - "Community 133"
Cohesion: 0.14
Nodes (14): modulesDir, projects, config, checks, data, home, modulesDir, path (+6 more)

### Community 134 - "Community 134"
Cohesion: 0.14
Nodes (14): assignmentStore, localUser, participants, updatedAt, id, name, ticketStore, collaboration (+6 more)

### Community 135 - "Community 135"
Cohesion: 0.31
Nodes (12): extract_packets(), _make_app(), Chatboks, Path, test_append_message_persists_packet_jsonl(), test_append_message_preserves_anchored_packet_observations(), test_confirmation_packet_context_marks_executor_output(), test_confirmation_packet_context_marks_verifier_review_and_repair() (+4 more)

### Community 136 - "Community 136"
Cohesion: 0.17
Nodes (13): paperSleuth, paperSleuth, exists, openTicketCount, projectFolder, projectRecentlyCompleted, projectTicketsPath, recentlyCompleted (+5 more)

### Community 137 - "Community 137"
Cohesion: 0.11
Nodes (20): collaboration, dropToken, dropTokenConfigured, dropZone, enabled, endpoint, peerName, reservations (+12 more)

### Community 138 - "Community 138"
Cohesion: 0.26
Nodes (12): _make_app(), Chatboks, Path, Smoke tests: slash commands bypass buffer_or_complete_input., All known slash commands return immediately without buffering., A slash command typed mid-composition leaves the existing buffer intact., Non-slash text without terminal punctuation is buffered as before., handle_user_input executes slash local commands end-to-end. (+4 more)

### Community 140 - "Community 140"
Cohesion: 0.20
Nodes (10): brief, generatedAt, headline, items, metrics, collaboratorsOnline, dirtyWorktrees, handoffs (+2 more)

### Community 141 - "Community 141"
Cohesion: 0.22
Nodes (9): appControl, available, command, label, mode, pid, processName, running (+1 more)

### Community 142 - "Community 142"
Cohesion: 0.25
Nodes (8): git, aheadBehind, branch, clean, commits, remoteUrl, status, uncommitted

### Community 143 - "Community 143"
Cohesion: 0.39
Nodes (6): _make_app(), Chatboks, Path, Smoke tests for local CLI latency summaries., test_latency_reports_no_records_yet(), test_latency_summary_reads_recent_jsonl_records()

### Community 144 - "Community 144"
Cohesion: 0.46
Nodes (7): _make_app(), Chatboks, Path, Smoke tests for manual collaboration outcome tracking., test_outcomes_summary_reads_jsonl(), test_suggest_outcome_uses_coordinator_without_recording_jsonl(), test_win_command_records_jsonl_without_agent_round()

### Community 145 - "Community 145"
Cohesion: 0.39
Nodes (6): _make_app(), Chatboks, Path, Smoke tests for native ChatBoks skill discovery., test_skills_command_lists_native_skills_without_agent_round(), test_skills_command_previews_requested_skill()

### Community 146 - "Community 146"
Cohesion: 0.09
Nodes (22): attention, customModules, dashboardMarkdownExists, files, generatedAt, graphify, configured, issue (+14 more)

### Community 147 - "Community 147"
Cohesion: 0.33
Nodes (6): fileTree, children, name, path, type, uncommitted

### Community 148 - "Community 148"
Cohesion: 0.20
Nodes (10): brief, generatedAt, headline, items, metrics, collaboratorsOnline, dirtyWorktrees, handoffs (+2 more)

### Community 149 - "Community 149"
Cohesion: 0.40
Nodes (5): procedures, close, dir, json, startup

### Community 150 - "Community 150"
Cohesion: 0.40
Nodes (5): project, id, name, path, ticketFolder

### Community 151 - "Community 151"
Cohesion: 0.40
Nodes (5): stats, files, languages, linesOfCode, sessions

### Community 152 - "Community 152"
Cohesion: 0.12
Nodes (16): flows, program, sessionClose, sessionStart, edges, nodes, positions, title (+8 more)

### Community 153 - "Community 153"
Cohesion: 0.22
Nodes (9): appControl, available, command, label, mode, pid, processName, running (+1 more)

### Community 154 - "Community 154"
Cohesion: 0.25
Nodes (8): git, aheadBehind, branch, clean, commits, remoteUrl, status, uncommitted

### Community 155 - "Community 155"
Cohesion: 0.29
Nodes (7): dropZone, enabled, inbox, incomingCount, outbox, outgoingCount, path

### Community 156 - "Community 156"
Cohesion: 0.33
Nodes (6): fileTree, children, name, path, type, uncommitted

### Community 157 - "Community 157"
Cohesion: 0.40
Nodes (5): procedures, close, dir, json, startup

### Community 158 - "Community 158"
Cohesion: 0.40
Nodes (5): project, id, name, path, ticketFolder

### Community 159 - "Community 159"
Cohesion: 0.40
Nodes (5): stats, files, languages, linesOfCode, sessions

### Community 160 - "Community 160"
Cohesion: 0.50
Nodes (4): codeGraph, configured, issue, status

### Community 161 - "Community 161"
Cohesion: 0.50
Nodes (4): graphify, configured, issue, status

### Community 162 - "Community 162"
Cohesion: 0.50
Nodes (4): graphify, configured, issue, status

### Community 163 - "Community 163"
Cohesion: 0.50
Nodes (4): graphify, configured, issue, status

### Community 164 - "Community 164"
Cohesion: 0.14
Nodes (14): modulesDir, projects, config, checks, data, home, modulesDir, path (+6 more)

### Community 165 - "Community 165"
Cohesion: 0.14
Nodes (14): assignmentStore, localUser, participants, updatedAt, id, name, ticketStore, collaboration (+6 more)

### Community 166 - "Community 166"
Cohesion: 0.17
Nodes (13): paperSleuth, paperSleuth, exists, openTicketCount, projectFolder, projectRecentlyCompleted, projectTicketsPath, recentlyCompleted (+5 more)

### Community 167 - "Community 167"
Cohesion: 0.18
Nodes (13): collaboration, dropToken, dropTokenConfigured, enabled, endpoint, peerName, reservations, reservationStore (+5 more)

### Community 168 - "Community 168"
Cohesion: 0.20
Nodes (10): brief, generatedAt, headline, items, metrics, collaboratorsOnline, dirtyWorktrees, handoffs (+2 more)

### Community 169 - "Community 169"
Cohesion: 0.22
Nodes (9): appControl, available, command, label, mode, pid, processName, running (+1 more)

### Community 170 - "Community 170"
Cohesion: 0.25
Nodes (8): git, aheadBehind, branch, clean, commits, remoteUrl, status, uncommitted

### Community 171 - "Community 171"
Cohesion: 0.29
Nodes (7): dropZone, enabled, inbox, incomingCount, outbox, outgoingCount, path

### Community 172 - "Community 172"
Cohesion: 0.29
Nodes (7): dropZone, enabled, inbox, incomingCount, outbox, outgoingCount, path

### Community 173 - "Community 173"
Cohesion: 0.33
Nodes (6): fileTree, children, name, path, type, uncommitted

### Community 174 - "Community 174"
Cohesion: 0.40
Nodes (5): procedures, close, dir, json, startup

### Community 175 - "Community 175"
Cohesion: 0.40
Nodes (5): project, id, name, path, ticketFolder

### Community 176 - "Community 176"
Cohesion: 0.40
Nodes (5): stats, files, languages, linesOfCode, sessions

### Community 177 - "Community 177"
Cohesion: 0.50
Nodes (4): codeGraph, configured, issue, status

### Community 178 - "Community 178"
Cohesion: 0.50
Nodes (4): codeGraph, configured, issue, status

## Knowledge Gaps
- **905 isolated node(s):** `version`, `configurations`, `allow`, `Path`, `Any` (+900 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **16 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Chatboks` connect `Community 44` to `Community 0`, `Community 1`, `Community 2`, `Community 3`, `Community 4`, `Community 135`, `Community 8`, `Community 10`, `Community 11`, `Community 12`, `Community 13`, `Community 138`, `Community 143`, `Community 144`, `Community 145`, `Community 45`, `Community 47`, `Community 49`, `Community 51`, `Community 52`, `Community 53`, `Community 65`, `Community 67`, `Community 73`, `Community 80`, `Community 81`, `Community 82`, `Community 85`, `Community 108`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **Why does `Stream` connect `Community 3` to `Community 0`, `Community 11`, `Community 13`, `Community 44`, `Community 46`, `Community 47`, `Community 49`, `Community 51`, `Community 56`, `Community 58`, `Community 65`, `Community 67`, `Community 73`, `Community 77`, `Community 80`, `Community 81`, `Community 82`, `Community 85`, `Community 87`, `Community 89`, `Community 124`?**
  _High betweenness centrality (0.049) - this node is a cross-community bridge._
- **Why does `Router` connect `Community 4` to `Community 0`, `Community 1`, `Community 67`, `Community 6`, `Community 7`, `Community 10`, `Community 11`, `Community 44`, `Community 47`, `Community 80`, `Community 82`, `Community 85`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Are the 47 inferred relationships involving `Chatboks` (e.g. with `BaseHTTPRequestHandler` and `ContextBuilder`) actually correct?**
  _`Chatboks` has 47 INFERRED edges - model-reasoned connections that need verification._
- **Are the 40 inferred relationships involving `Stream` (e.g. with `AgentTimeoutError` and `BaseHTTPRequestHandler`) actually correct?**
  _`Stream` has 40 INFERRED edges - model-reasoned connections that need verification._
- **Are the 22 inferred relationships involving `CoordinatorAgent` (e.g. with `BaseAgent` and `ContextBuilder`) actually correct?**
  _`CoordinatorAgent` has 22 INFERRED edges - model-reasoned connections that need verification._
- **Are the 24 inferred relationships involving `Router` (e.g. with `AgentTimeoutError` and `Chatboks`) actually correct?**
  _`Router` has 24 INFERRED edges - model-reasoned connections that need verification._